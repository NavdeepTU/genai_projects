...
def _service(db_session) -> AuthService:
    return AuthService(UserRepository(db_session), SessionRepository(db_session), TenantRepository(db_session))
...
async def test_sign_up_creates_a_user_with_a_hashed_password(db_session):
    ...
    user = await service.sign_up("alice@example.com", "correct horse battery staple", tenant_id)
...
async def test_sign_up_rejects_a_duplicate_email(db_session):
    service = _service(db_session)
    tenant_id = await _tenant_id(db_session)
    await service.sign_up("alice@example.com", "first-password", tenant_id)

    with pytest.raises(EmailAlreadyRegisteredError):
        await service.sign_up("alice@example.com", "second-password", tenant_id)


async def test_sign_up_rejects_an_unknown_tenant(db_session):
    service = _service(db_session)

    with pytest.raises(TenantNotFoundError):
        await service.sign_up("alice@example.com", "correct horse battery staple", uuid.uuid4())
...
async def test_get_user_by_token_returns_none_for_an_expired_session(db_session):
    ...
    tenant_id = await _tenant_id(db_session)
    user = await user_repository.create_user("alice@example.com", "hashed", tenant_id)
